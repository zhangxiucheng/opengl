...
void main()
{
    ...
    if (chosen == 0)
        FragColor = vec4(texture(tx, fs_in.tc).rgb * Blinn_Phong(nlightDir, viewDir, fs_in.norm, .2f, 64.f, shadow), float(code) / 255.f);
    else
        FragColor = mix(vec4(texture(tx, fs_in.tc).rgb * Blinn_Phong(nlightDir, viewDir, fs_in.norm, .2f, 64.f, shadow), float(code) / 255.f), fc, rt);
}